...
tempss gestion_temps_decompteur(int pause_time_disc,int pause_time_pause ,tempss temp,SDL_Surface *ecran,int delay)
{
 ...
 if((pause_time_disc==0)&&(pause_time_pause==0)&&(temp.maxtemp!=0))     
   {    
          ...
          //SDL_FillRect(ecran, NULL, SDL_MapRGB(ecran->format, 255, 255, 255));    
    temp.tempactuel = SDL_GetTicks();
        if (temp.tempactuel - temp.tempprecedent >=delay)
        {
            temp.seconde1-=1;
            temp.maxtemp--; 
            printf("max temp ====>  %d <===\n",temp.maxtemp);  
